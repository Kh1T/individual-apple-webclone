...
get
    pinInfo.forEach((title, index) => {
      ...
      element.innerHTML = `
                ...
                <div id="pinwheel${index + 1}-carousel"></div>
...
class PinwheelCarousel extends HTMLElement {
  ...
  connectedCallback() {
    const dataArray = this.getAttribute("data-array");
    if (!dataArray) {
      console.error("data-array attribute is missing or invalid.");
      return;
    }

    const parsedDataArray = JSON.parse(dataArray);
    const totalSlides = parsedDataArray.length;
    this.className =
    ...
    parsedDataArray.forEach((_, index) => {
    ...
    this.initializeCarouselButtons();
  }
  ...
  initializeCarouselButtons() {
    ...
    buttons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        this.updateCarousel(slideIndex, buttons);
      });
    });
  ...
  updateCarousel(slideIndex, buttons) {
    const offset = -(slideIndex * 100);
    const totalSlides = buttons.length;
    const initialOffset = (Math.floor(totalSlides / 2) - 1) * 10;
    this.carouselItems.style.transform = `translateX(${offset / 10 + 5 + initialOffset}%)`;
...
const pinInfo2 = [
  ...
  },
  {
    ...
    title: "OtterBox Lumen Series Case with MagSafe for iPhone 15 Plus - Green",
    price: "49.95",
  },
];
const pinInfo3 = [
...
const pinWheelData = [pinInfo1, pinInfo2, pinInfo3];

pinWheelData.forEach((data, index) => {
  // Create the pinwheel-Carousel element
  const element = document.createElement("pinwheel-Carousel");

  // Set the attribute 'data-array' with the JSON stringified data
  element.setAttribute("data-array", JSON.stringify(data));

  // Get the parent element and append the created element to it
  const parentElement = document.getElementById(
    `pinwheel${index + 1}-carousel`,
  );
  if (parentElement) {
    parentElement.appendChild(element);
  } else {
    console.error(`Element with ID 'pinwheel${index + 1}-carousel' not found.`);
  }
});
